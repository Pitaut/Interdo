...
SELECT 
    ...
    CASE 
        WHEN fhf.id IS NOT NULL THEN ROUND(fhf.montant_total - IFNULL(r.cout_total, 0), 2)
        ELSE -IFNULL(r.cout_total, 0)
    END AS marge_brute,
    
    CASE 
        WHEN fhf.id IS NOT NULL AND fhf.montant_total > 0 
        THEN ROUND(((fhf.montant_total - IFNULL(r.cout_total, 0)) / fhf.montant_total) * 100, 2)
        ELSE NULL 
